...
# 	 		then sbatch --partition=LONG  --wrap="samtools sort -n -T ${line}_temp -O BAM -o ${line}_sorted.bam ${line}";
...
# 		echo "$name ... $percorso";
# 		if [[ ! -a ${line}.fq2 ]]; 
# 			then sbatch --partition=LONG  --wrap="bamToFastq -i ${line}_sorted.bam -fq $percorso$name.fq1 -fq2 $percorso$name.fq2";
...
cat bams_for_mpileup | while read line
 	do 	percorso=`echo ${line} | sed "s|[A-Z_]*[0123456789b]*.[a-z]*.bam||g"`
        ...
        sbatch --partition=LONG --dependency=afterok:${PROC} --wrap="${command2}"
	done
waitforcompletion								   																												
