...
cat bams_for_mpileup | while read line
 	do 	percorso=`echo ${line} | sed "s|/.*\.bam|/|g"`
		...
		export SBATCH_CMD="bwa mem -R \"@RG\tID:$name\tSM:$name\" $DIR/../mpileup_defaults/repDNA_ref/repDNA.fa $percorso$name.fq1.gz $percorso$name.fq2.gz | samtools view -b -F 4 - | bamsormadup threads=16 SO=coordinate level=0 verbose=0 fixmate=1 adddupmarksupport=1 tmpfile=$name.tmp | samtools sort > TR_BAMS/$name.Ty.bam"
